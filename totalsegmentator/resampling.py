...
def change_spacing(img_in, new_spacing=1.25, target_shape=None, order=0, nr_cpus=1,
                   ...
                   use_gpu=False, torch_resample=False, device="cpu"):
    """
    ...
    # Plain nearest label upsample (order 0, no -ho) stays on scipy: it is an
    # upsample (no aliasing to fix), nearest gains nothing, and its coarse input
    # is too small for the GPU transfer to pay off.
    _use_torch = torch_resample and data.ndim == 3 and (nnunet_resample or order != 0)
    if _use_torch:
        _dev = device.type if hasattr(device, "type") else str(device)
        if _dev not in ("mps", "cuda", "cpu"):
            _dev = "cpu"
        if target_shape is not None:
